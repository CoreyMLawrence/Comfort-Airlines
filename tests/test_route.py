...
def test_route_init(airport, hub) -> None:
    route = Route(AircraftType.AIRBUS_A200_100, airport, hub, 1.0, 1, 1.0)

def test_route_init_aircraft_illegal_type(airport, hub) -> None:
    with pytest.raises(TypeError):
        _ = Route("not an AircraftType object", airport, hub, 1.0, 1, 1.0)

def test_route_init_source_airport_illegal_type(hub) -> None:
    with pytest.raises(TypeError):
        _ = Route(AircraftType.AIRBUS_A200_100, "not an Airport object", hub, 1.0, 1, 1.0)

def test_route_init_destination_airport_illegal_type(airport) -> None:
    with pytest.raises(TypeError):
        _ = Route(AircraftType.AIRBUS_A200_100, airport, "not an Airport object", 1.0, 1, 1.0)

def test_route_init_same_airports(airport) -> None:
    with pytest.raises(ValueError):
        _ = Route(AircraftType.AIRBUS_A200_100, airport, airport, 1.0, 1, 1.0)

@pytest.mark.parametrize("distance", [0.01, 103.13, 351013.0])
def test_route_init_distance_legal_value(distance, airport, hub) -> None:
    _ = Route(AircraftType.AIRBUS_A200_100, airport, hub, distance, 1, 1.0)
...
def test_route_init_demand_legal_value(demand, airport, hub) -> None:
    _ = Route(AircraftType.AIRBUS_A200_100, airport, hub, 1.0, demand, 1.0)
...
def test_route_init_fuel_requirement_legal_value(fuel_requirement, airport, hub) -> None:
    _ = Route(AircraftType.AIRBUS_A200_100, airport, hub, 1.0, 1, fuel_requirement)
